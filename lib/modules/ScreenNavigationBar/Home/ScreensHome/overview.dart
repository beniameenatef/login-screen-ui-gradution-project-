...
class _HomeoverviewState extends State<Homeoverview> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: Future.wait([library,year,astaff,mstaff,lab,studentdistriubtion,graduatednumber]),
...
قواعد بيانات شئون اعضاء الهيئة التدريسية-
                    ...
                    ''',
                    textAlign: TextAlign.end,),
                  Text("الجهاز الإدارى  ",style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold),),
                  Text("عدد العاملين بالجهاز الإداري حسب المهام الوظيفية   ",style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold,color: AppColors.blue),textAlign: TextAlign.center,),
                  SizedBox(height: 10,),
                  DataTable(
                    columnSpacing: 38.0,
                    columns: [
                    ...
                    rows: List.generate(1, (index) {
                      ...
                      return DataRow(cells: [
                        ...
                        DataCell(Container(width: 75, child: Text('الفنين وأمناء معامل الحاسب '))),
                      ]);
                    }),),
                  SizedBox(height: 20,),
                  Text("الموارد المالية والمادية   ",style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold),),
                  Text("الميزانية المالية السنوية للكلية   ",style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold,color: AppColors.blue),textAlign: TextAlign.center,),
...
وأهدافها الاستراتيجية  
                    ...
                    textAlign: TextAlign.end,),
                  SizedBox(height: 10,),
                  Text(":  التسهيلات المادية الداعة للعملية التعليمية    ",style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold,color: AppColors.blue),textAlign: TextAlign.center,),
                  SizedBox(height: 10,),
                  Text('''
...
تقوم الكلية وبصفة دورية بتجديد وتحديث معامل الحاسب بها ضمن خطتها السنوية لدعم وصيانة البنية التحتية والمرافق لضمان استمرارية تحقيق فرص التعلم والتطبيق العملي لمخرجات التعلم المستهدفة بالمقررات الدراسية.
                    ...
                    textAlign: TextAlign.end,),
                  SizedBox(height: 10,),
                  Text("بيان بعدد أجهزة الحاسب الآلي في الكلية    ",style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold,color: AppColors.blue),textAlign: TextAlign.center,),
                  DataTable(
                    ...
                    List.generate(snapshot.data![4]!.data!.length, (index) {
                      final y = snapshot.data![4]!.data![index].attributes!.labNumber.toString();
                      final x = snapshot.data![4].data![index].attributes!.pCnumber.toString();
                  ...
                  SizedBox(height: 30,),
                  Divider(),
                  Text("الطلاب والخريجون     ",style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold,color: AppColors.blue),textAlign: TextAlign.center,),
                  Text("قواعد القبول والتحويل ونظام الدراسة، والخدمات الجامعية المقدمة     ",style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold,color: AppColors.blue),textAlign: TextAlign.end,),
                  SizedBox(height: 10,),
                  Text('''
تم  إدراج هذه القواعد ضمن محتويات دليل الطالب، والذي يتضمن التعريف بالكلية وأقسامها والفرق بين هذه
 ...
 كما يتم نشره على موقع الكلية الإلكتروني
                    ...
                    textAlign: TextAlign.end,),
                  SizedBox(height: 10,),
                  Text("إجمالي الطلاب بالمستويات الدراسية     ",style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold,color: AppColors.blue),textAlign: TextAlign.justify,),
                  DataTable(
                    ...
                    List.generate(4, (index) {
                      final b = snapshot.data![5].data![index].attributes!.level;
                      final h = int.parse(snapshot.data![5].data![index].attributes!.male);
                      final i = int.parse(snapshot.data![5].data![index].attributes!.female);
                  ...
                  SizedBox(height: 30,),
                  Text("تطور اعداد الخريجين      ",style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold,color: AppColors.blue),textAlign: TextAlign.center,),
                  Text("إحصائية تطور أعداد الخريجين خلال خمس سنوات     ",style: TextStyle(fontSize: 15,fontWeight: FontWeight.bold,color: AppColors.blue),textAlign: TextAlign.justify,),
                  SizedBox(height: 10,),
                  DataTable(
                    ...
                    columns: const [
                      ...
                      DataColumn(label: Text(' ')),
                      DataColumn(label: Text('علوم الحاسب',overflow: TextOverflow.ellipsis,)),
                      DataColumn(label: Text(' ')),
                      DataColumn(label: Text('نظم المعلومات',overflow: TextOverflow.ellipsis,)),
                      DataColumn(label: Text(' ')),
                    ...
                    List.generate(snapshot.data![6].data!.length, (index) {
                      ...
                      dynamic y = int.parse(snapshot.data![6].data![index].attributes!.cs!.number);
                      dynamic z = int.parse(snapshot.data![6].data![index].attributes!.attributesIs!.number);
